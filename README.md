...
**实体对齐**：22 条标注集上 Precision 100% / Recall 100% / F1 100%（改进前 90%），
...
- **住宿推荐与检索**：`/hotel/recommend` 打开即推荐附近酒店（无需关键词），`/hotel/search` 用关键词收窄，两者都支持 `page` 分页；POI 查询新增 `types`（住宿类别过滤）、`extensions`（取回评分等）与偏移分页；结果统一规整并做**档位归一**（经济型 / 舒适型 / 高档型 / 豪华型 / 民宿）。`PlanRequest.hotel` 会**透传并回显**——「先选酒店再规划」不再被前端门槛拦住
...
- Leaflet 地图按天分色路线（**7 个不同色相**：蓝 / 橙 / 青绿 / 紫 / 玫红 / 黄绿 / 靛，地图、图例、时间线与总览共用同一套配色）、图例开关、住宿锚点标记；底图可切换高德 / OSM（含 GCJ-02↔WGS84 转换）
- 景点详情卡：实景图灯箱、AI 介绍、好评 / 避雷双卡、地址一键唤起高德导航
- **酒店选择器**：打开即推荐附近酒店（携程式，无需先搜索），可搜索收窄，**分页「加载更多」**（每页 25 条）；卡片给出评分、**档位**（经济型 / 舒适型 / 高档型 / 豪华型 / 民宿）、区域、**到行程中心的真实距离**、地址、电话与多图画廊，支持「推荐 / 距离近→远 / 评分优先」排序与档位、评分筛选，一键更换（当前酒店高亮），点名称就地展开小地图（橙点=酒店、彩点=当前行程景点）判断住宿是否顺手；**没有真实房价**——高德免费接口不含房价，字段留空即不显示，**绝不编造**
...
- **分享长图**：Canvas 手绘把整份行程导成竖版 PNG（含 AI 总评），**先弹预览浮层再「下载 / 取消」**（原先点一下就静默下载）；**零依赖、不引截图库**（地图刻意不画：高德静态图会跨域污染画布，`toBlob` 抛 SecurityError）
- 规划进度条（原进度只写进折叠日志，看着像卡住）、行程列表缩略图、右上角城市点击切换
- **modern-minimal 视觉统一**：左栏表单、右栏结果区、「我的」页与日历弹层统一到 OpenDesign `modern-minimal` 方向（发丝描边、去卡片阴影、紧字距、等宽数字、字重收敛）
- **图标规范**：全站图标改为**线性 SVG**（统一 16 视窗、`currentColor`、1.8px 描边；酒店图标采用 Lucide 的 `hotel` 造型），替换了原来的 emoji —— 仅保留桌宠 🐋 形象与代码注释里的 emoji
- **用户面文案去黑话**：hero 副标题、说明 chip、识别提示改用白话；规划进度条的阶段名换成友好文案（折叠日志里仍保留原始阶段名）
...
| `LLM_FAST_API_KEY` / `LLM_FAST_BASE_URL` / `LLM_FAST_MODEL` | 轻任务快模型 | 评价生成；**可指向免费模型，不配则回落主模型** |
...
**填完先自检，再启服务** —— `python backend/selftest.py` 会真实打一次高德与 LLM，
把三个通道标成 `ok / invalid / unreachable / missing` 并给退出码，免得「改了配置重启了才发现 Key 错了」。
服务启动时也会在后台自检一次（结果缓存 6 小时），首页「服务状态」显示的就是这个**探测过的真实可用性**，
不是「配置文件里有个值」：

| 状态 | 含义 |
|------|------|
| `ok` | 探测通过，可用 |
| `invalid` | 已配置但被上游拒绝（401 / 额度 / 模型名错 / 快通道缺 Key） |
| `unreachable` | 探测时网络不通 —— **与 invalid 分开报**，修复动作完全相反 |
| `missing` | 未配置，走设计好的降级路径 |

`/health` 的 `amap` 字段只表示「Key 是否存在」（它给 Docker healthcheck 用），
`/meta` 的 `amap_key` 才是探测过的可用性 —— 这个分工是刻意的。
...
| GET | `/meta` | 服务元信息：运行时遍历 `app.routes` 生成的接口清单 + 服务状态 |
| GET | `/demo/spots` · `/demo/config` | 演示景点（含图片与介绍）/ 底图配置（城市表、无 Key 降级开关） |
| GET | `/cities` | 可演示城市列表 + 各城中心坐标（前端城市选择器数据源） |
| POST | `/extract` | 攻略文本 → LLM 抽取（含城市识别）→ 实体对齐 |
...
```bash
cd backend && python -m pytest -q          # 单元测试（266 passed；依赖高德的酒店用例无 AMAP_KEY 时跳过）
cd .. && node tools/frontend_smoke.js      # 前端 jsdom 运行时冒烟（14 项）
node tools/hotel_smoke.js                  # 酒店弹层 + 长图预览 + 地图视图 jsdom 冒烟（30 项）
python tools/check_frontend.py             # 前端静态检查（7 项阻塞门禁）
python tools/check_backend_health.py       # 后端五维健壮性审计（AST，无 Key 即可跑）
```

- **测试与 CI**：**266 个单元测试**全过（对齐 / 求解 / 校验 / 编辑器 / 任务流水线 / 稳健性 / 偏好 / 媒体 key / 城市错配守卫 / 天气 / 酒店接口），GitHub Actions 全绿
- **五道 CI 门禁**：单元测试 + **前端静态检查**（7 项阻塞门禁：语法 / 变量遮蔽 / 硬编码坐标 / 属性转义 / CSS 自引用 / 未定义 CSS 变量 / 文档完整性）+ **后端五维健壮性审计**（超时 / 重试 / 状态 / 日志 / 成本）+ **jsdom 运行时冒烟**（`frontend_smoke.js` 14 项 + `hotel_smoke.js` 30 项）+ 无密钥的求解器 demo
...
backend/
  ...
  editor.py               对话式编辑（意图解析 + 确定性执行 + 评价生成 + AI 总评）；POI 检索与酒店卡片规整 / 档位归一
...
- **v1.7（2026-09-23）**：**首页信息主次重构 + 两页字号字重阶梯**——把「5 张图平铺、读完不知道在说什么」改成「**结论先写、证据在后**」：核心论点（求解器与 LLM 质量分几乎持平 0.915 vs 0.921，但一个 0 违规、一个违规 8 次）提到首屏做 28px 结论句，主图独占版面拿全页唯一的一级阴影，其余 4 张收进原生 `<details>` 折叠（零 JS、可键盘展开、关 JS 也不白屏）；**图表语义配色**——图 1 里「本项目求解器」与「LLM 直接排期」原先用同一个蓝的深浅两档（全篇最重要的对比却最难分辨），改为**实心饱和蓝 / 实心中灰 / 空心浅灰**，区分交给色相与「实心与否」两个通道，转灰度也分得开；**字号收敛到 12/14/16/20/28/44/62 七档、字重补进 400 与 600**（原先前端字重全 ≥650，屏幕上没有一个「轻」的字），规划器 17 个字号取值收敛为 5 个；**留白分 128/96/64 三档**（原来 8 个区块共用 96px 一档，节奏是平的）；#results 占全页正文比例 **51.6% → 18.6%**；前端检查器新增**文档完整性**与 **Markdown 标记泄漏**两条守卫
- **v1.6（2026-09-23）**：**首页 footer 死链修复 + 实验数据图表**——footer 的「接口文档」原指向 `/docs`（FastAPI 自带 Swagger UI，样式与脚本从 CDN 加载，违反本站零 CDN 纪律）、「服务状态」指向 137 字节裸 JSON，两者都「点进去没有具体东西」；现在改为**站内区块**，数据来自新增的 `GET /meta` —— 服务端**运行时遍历 `app.routes`** 生成接口清单（不是手写第二份，从机制上杜绝漂移），顺带暴露并补齐了 4 个接口缺失的 docstring；新增 **5 张实验数据图表**（纯内联 SVG、零 JS，每个数据点标出精确数值，图 1 为三方排期对比）；首页↔规划器**双向跳转过渡**（含显式返回入口）；接口数 24 → **25**、测试 228 → **238**
