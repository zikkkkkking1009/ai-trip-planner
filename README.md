...
```bash
cd backend && python -m pytest -q          # 单元测试（286 passed；依赖高德的酒店用例无 AMAP_KEY 时跳过）
...
- **测试与 CI**：**286 个单元测试**全过（对齐 / 求解 / 校验 / 编辑器 / 任务流水线 / 稳健性 / 偏好 / 媒体 key / 城市错配守卫 / 天气 / 酒店接口），GitHub Actions 全绿
