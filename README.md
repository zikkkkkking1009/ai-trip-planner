...
共 **22 个接口**（10 GET / 10 POST / 1 DELETE / 1 WebSocket）。

---

## 工程实践

- **测试与 CI**：**195 个单元测试**全过（对齐 / 求解 / 校验 / 编辑器 / 任务流水线 / 稳健性 / 偏好 / 媒体 key / 城市错配守卫），GitHub Actions 全绿
- **五道 CI 门禁**：单元测试 + **前端静态检查**（语法门 + 变量遮蔽守卫）+ **后端五维健壮性审计**（超时/重试/状态/日志/成本）+ **jsdom 运行时冒烟**（12 项 DOM 断言）+ 无密钥的求解器 demo
...
```
backend/
  main.py                 FastAPI 入口与 22 个接口（21 HTTP + 1 WebSocket）
